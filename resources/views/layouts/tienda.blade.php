...
<body class="font-sans antialiased bg-gray-50 text-gray-800">
    <div class="min-h-screen flex flex-col">
        {{-- Header --}}
        <header class="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-50" x-data="{ mobileMenuOpen: false }">
                ...
                <div class="flex items-center justify-between h-16 md:h-20">
                    ...
                    <div class="hidden md:flex flex-1 max-w-lg mx-8">
                        <form action="{{ route('productos.catalogo') }}" method="GET" class="w-full">
                            <div class="relative">
                                ...
                                <button type="submit" class="absolute left-3 top-2.5 w-4 h-4 text-gray-400 hover:text-sky-600 transition-colors">
                    ...
                    <div class="flex items-center gap-3 md:gap-5">
                        <a href="{{ route('cart.index') }}" class="relative text-gray-600 hover:text-sky-600 transition-colors">
                            ...
                            @if($cartCount > 0)
                                <span class="absolute -top-2 -right-2 bg-pink-500 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">{{ $cartCount }}</span>
                            @endif
                        </a>

                        @if($configuracion->instagram || $configuracion->facebook || $configuracion->whatsapp)
                        <div class="hidden md:flex items-center gap-3 border-l border-gray-200 pl-4">
                            @if($configuracion->instagram)
                            <a href="{{ $configuracion->instagram }}" target="_blank" rel="noopener noreferrer" class="text-gray-500 hover:text-pink-500 transition-colors" aria-label="Instagram">
                            ...
                            <a href="{{ $configuracion->facebook }}" target="_blank" rel="noopener noreferrer" class="text-gray-500 hover:text-blue-500 transition-colors" aria-label="Facebook">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                            ...
                            <a href="https://wa.me/{{ $configuracion->whatsapp }}?text=Hola!%20Quiero%20consultar%20por%20productos" target="_blank" rel="noopener noreferrer" class="text-gray-500 hover:text-green-500 transition-colors" aria-label="WhatsApp">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                        ...
                        {{-- Mobile menu toggle --}}
                        <button @click="mobileMenuOpen = !mobileMenuOpen" class="md:hidden text-gray-600 hover:text-sky-600">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {{-- Mobile menu --}}
            <nav x-show="mobileMenuOpen" @click.outside="mobileMenuOpen = false" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" class="md:hidden border-t border-gray-100 bg-white">
                <div class="px-4 py-3 space-y-1">
                    <a href="{{ route('home') }}" class="block px-3 py-2 text-sm font-medium text-gray-600 hover:text-sky-600 hover:bg-sky-50 rounded-lg {{ request()->routeIs('home') ? 'text-sky-600 bg-sky-50' : '' }}">
                        Inicio
                    </a>
                    <a href="{{ route('productos.catalogo') }}" class="block px-3 py-2 text-sm font-medium text-gray-600 hover:text-sky-600 hover:bg-sky-50 rounded-lg {{ request()->routeIs('productos.catalogo') ? 'text-sky-600 bg-sky-50' : '' }}">
                        Todos los productos
                    </a>
                    @foreach($categoriasNav as $cat)
                        <a href="{{ route('productos.categoria', $cat) }}" class="block px-3 py-2 text-sm font-medium text-gray-600 hover:text-sky-600 hover:bg-sky-50 rounded-lg {{ request()->routeIs('productos.categoria') && request()->route('categoria')?->id === $cat->id ? 'text-sky-600 bg-sky-50' : '' }}">
                            {{ $cat->nombre }}
                        </a>
                    @endforeach
                    <a href="{{ route('contacto.index') }}" class="block px-3 py-2 text-sm font-medium text-gray-600 hover:text-sky-600 hover:bg-sky-50 rounded-lg {{ request()->routeIs('contacto.*') ? 'text-sky-600 bg-sky-50' : '' }}">
                        Contacto
                    </a>
        ...
        <footer class="bg-gray-800 text-gray-300 mt-12">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div>
                        <div class="flex items-center gap-2 mb-4">
                            ...
                            <span class="text-lg font-bold text-white">{{ $configuracion->nombre_negocio ?? 'Pañalera' }}</span>
                        </div>
                        <p class="text-sm leading-relaxed">{{ $configuracion->descripcion ?? 'Tu tienda de confianza para el cuidado de tu bebé.' }}</p>
                    </div>

                    @if($configuracion->email || $configuracion->telefono || $configuracion->instagram || $configuracion->facebook)
                    <div>
                        ...
                        <ul class="space-y-2 text-sm">
                            ...
                            <li class="flex items-center gap-2">
                                ...
                                <a href="{{ $configuracion->instagram }}" target="_blank" rel="noopener noreferrer" class="text-gray-400 hover:text-pink-300 transition-colors">Instagram</a>
                            </li>
                            @endif
                            @if($configuracion->facebook)
                            <li class="flex items-center gap-2">
                                ...
                                <a href="{{ $configuracion->facebook }}" target="_blank" rel="noopener noreferrer" class="text-gray-400 hover:text-blue-300 transition-colors">Facebook</a>
                            </li>
                            @endif
                        </ul>
                    </div>
                    ...
                    <div>
                        ...
                        <ul class="space-y-2 text-sm">
                            @if($configuracion->whatsapp)
                            <li>
                                <a href="https://wa.me/{{ $configuracion->whatsapp }}?text=Hola!%20Quiero%20consultar%20por%20productos" target="_blank" rel="noopener noreferrer" class="flex items-center gap-2 text-green-400 hover:text-green-300 transition-colors">
                ...
                <div class="border-t border-gray-700 mt-8 pt-6 text-center text-sm text-gray-500">
                    &copy; {{ date('Y') }} {{ $configuracion->nombre_negocio ?? 'Pañalera' }}. Todos los derechos reservados.
                </div>
            </div>
        </footer>
    </div>
    ...
    <a href="https://wa.me/{{ $configuracion->whatsapp ?? '541112345678' }}?text=Hola!%20Quiero%20consultar%20por%20productos"
